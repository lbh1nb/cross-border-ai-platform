...
**当前进度**：第 2 周第 5 天（08-07），已完成 11 大功能模块 + 桌面 GUI + exe 打包
...
| **PySide6 桌面 GUI** | ✅ 已完成 | 配置/审批流管理/任务控制/数据看板 4 大面板 |
...
```mermaid
flowchart LR
    A[主窗口] --> B[📋 配置面板<br/>飞书凭证/表ID/审批人]
    A --> C[✅ 审批流管理<br/>扫描/选择/一键启用]
    A --> D[⚙️ 任务控制<br/>启停调度器/查看日志]
    A --> E[📊 数据看板<br/>选品池/库存预警]

    B --> F[自动写入 .env<br/>业务用户不接触代码]
    C --> G[自动检测飞书审批定义<br/>自动查字段ID和节点ID]
    D --> H[后台运行调度器<br/>实时显示日志]
...
**方式 A：在 GUI 里一键启用（推荐，业务用户适用）**

1. 打开 GUI（双击 `scripts\启动GUI.bat` 或运行 `跨境电商AI运营中台.exe`）
2. 进入"审批流管理"页面
3. 点"🔍 扫描审批定义" → GUI 自动列出企业内所有审批定义
4. 选中目标审批 → GUI 自动查询字段 ID 和节点 ID
5. 点"✅ 启用此审批流" → 配置自动写入 .env
6. 启用后每天 10:00 自动触发

**方式 B：手动配置 .env（IT 人员适用）**

详见 .env.example 审批流章节：
...
业务用户可以随时在飞书审批后台创建新的审批流，然后在 GUI 里一键启用，全程不接触代码。
...
1. 双击 `scripts\启动GUI.bat` 启动 GUI
2. 进入"审批流管理"页面
3. 点"🔍 扫描审批定义" → 刚创建的审批会自动出现在列表里
4. 选中刚创建的审批 → 右侧自动显示详情（字段数、节点 ID 等）
5. 点"✅ 启用此审批流" → 弹出确认框 → 点"是"
6. 状态变为"已启用 ✓"

#### 第三步：配置触发条件

进入"配置"页面，调整以下参数：
- **审批触发金额**：默认 $5000，可修改（如改为 $3000）
- **审批人 Open ID**：填入审批人的 `ou_` 开头的 open_id

#### 第四步：启动调度器

1. 进入"任务控制"页面
2. 点"▶ 启动调度器"
3. 每天 10:00 会自动扫描选品池，金额超过阈值的商品自动创建审批实例
4. 审批人在飞书审批中心收到通知 → 通过/拒绝 → 状态自动回写表格

#### 创建多个审批流

如果需要多个审批流（如"采购审批"+"补货审批"），重复上述步骤即可。每次切换审批流时：
1. 在飞书创建新的审批定义
2. 在 GUI 扫描 → 选中 → 启用（会覆盖之前的配置）
3. 新的审批流立即生效

#### 审批流字段说明

飞书审批定义的字段 ID 是自动生成的（如 `widget17850667532920001`），GUI 会自动查询并配置，无需手动填写。
...
│       ├── pages/             # 4 个页面
│       │   ├── config_page.py     # 配置面板
│       │   ├── approval_page.py   # 审批流管理
│       │   ├── task_page.py       # 任务控制
│       │   └── dashboard_page.py  # 数据看板
│       └── services/          # 服务层
│           ├── env_service.py      # .env 读写
│           └── approval_service.py # 审批流扫描/启用
├── scripts/                   # 运维脚本
